        ...
        <div className="bg-background border border-border rounded-lg modal-shadow w-full max-w-2xl max-h-[90vh] flex flex-col animate-fade-in">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            <div>
              ...
              <input
                type="file"
                accept=".jpg,.png,.pdf"
                className="w-full text-xs text-muted-foreground file:mr-2 file:px-3 file:py-1 file:rounded-md file:border-0 file:bg-accent file:text-accent-foreground file:text-xs file:font-medium"
              />
                ...
                <div key={i} className="rounded-md border border-border p-3 space-y-3 relative">
                  {lines.length > 1 && (
                    <button
                      ...
                      className="absolute top-2 right-2 p-1 rounded hover:bg-destructive/10 text-destructive transition-colors"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <select
                        value={line.category}
                        onChange={(e) => updateLine(i, { category: e.target.value, brand: "", customBrand: "" })}
                        className="w-full px-3 py-1.5 rounded-md border border-border bg-background text-sm"
                      >
                    ...
                    <div>
                      ...
                      {line.brand === "Other" && (
                        <input
                          type="text"
                          value={line.customBrand}
                          onChange={(e) => updateLine(i, { customBrand: e.target.value })}
                          placeholder="Enter brand name"
                          className="w-full mt-1 px-3 py-1.5 rounded-md border border-border bg-background text-sm placeholder:text-muted-foreground/60"
                        />
                      )}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateLine(i, { unit_buying_price: parseFloat(e.target.value) || 0 })}
                        placeholder="Enter buying price (ETB)"
                        className="w-full px-3 py-1.5 rounded-md border border-border bg-background text-sm placeholder:text-muted-foreground/60"
                      />
                    </div>
                  </div>
          ...
          <div className="flex items-center justify-between px-5 py-4 border-t border-border shrink-0">
            <div className="text-sm text-muted-foreground">
              Total Cost: <span className="font-semibold text-foreground">{totalCost.toLocaleString()} ETB</span>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={onClose} className="px-4 py-1.5 rounded-md border border-border text-sm font-medium hover:bg-accent transition-colors">
                Cancel
              </button>
